...
AND (CASE
    WHEN @TabName = 'BILLING' THEN 
        CASE WHEN (CAST(POD.U_PODStatusDetail as nvarchar(max)) LIKE '%Verified%' OR CAST(POD.U_PODStatusDetail as nvarchar(max)) LIKE '%ForAdvanceBilling%') THEN 1
        ELSE 0 END
    WHEN @TabName = 'TP' THEN 
        CASE WHEN CAST(POD.U_PODStatusDetail as nvarchar(max)) LIKE '%Verified%' THEN 1
        ELSE 0 END
    ELSE 1
END) = 1
;  
