...
BEGIN
    ...
    SELECT
        ...
        CAST((SELECT value FROM @GrossClientRatesTax WHERE id = POD.U_BookingNumber) AS NUMERIC(19,6)) AS bi_U_GrossInitialRate,
        ISNULL(PRICING.U_Demurrage, 0) AS bi_U_Demurrage,
        ISNULL(PRICING.U_Demurrage2, 0) AS tp_U_Demurrage,
        ISNULL(PRICING.U_Demurrage, 0) AS pr_U_Demurrage,
        ...
        CAST((SELECT value FROM @TotalAddtlCharges WHERE id = POD.U_BookingNumber) AS NUMERIC(19,6)) AS pr_U_TotalAddtlCharges,
        ISNULL(PRICING.U_Demurrage2, 0) AS pr_U_Demurrage2,
        ISNULL(PRICING.U_AddtlDrop2, 0) AS pr_U_AddtlDrop2,
        ISNULL(PRICING.U_Manpower2, 0) AS pr_U_Manpower2,
        ISNULL(PRICING.U_Backload2, 0) AS pr_U_Backload2,
        ISNULL(PRICING.U_AddtlDrop2, 0) 
        + ISNULL(PRICING.U_BoomTruck2, 0) 
        + ISNULL(PRICING.U_Manpower2, 0) 
        + ISNULL(PRICING.U_Backload2, 0) AS pr_U_totalAddtlCharges2,
        CASE
            WHEN ISNULL(trucker.VatStatus,'Y') = 'Y' THEN ISNULL(PRICING.U_Demurrage2, 0)
            WHEN ISNULL(trucker.VatStatus,'Y') = 'N' THEN (ISNULL(PRICING.U_Demurrage2, 0) / 1.12)
        END AS pr_U_Demurrage3,
        ((CASE
            WHEN ISNULL(client.VatStatus,'Y') = 'Y' THEN ISNULL(PRICING.U_Demurrage, 0)
            WHEN ISNULL(client.VatStatus,'Y') = 'N' THEN (ISNULL(PRICING.U_Demurrage, 0) / 1.12)
        END) + (CASE
        ...
        END AS tp_U_AddtlChargesN,
        ISNULL(TP.U_ActualRates, 0) AS tp_U_ActualRates,
        ...
        ISNULL(TP.U_ActualCharges, 0) AS tp_U_ActualCharges,
        ISNULL(TP.U_OtherCharges, 0) AS tp_U_OtherCharges,
        ...
        BILLING.U_RateAdjustments AS pr_U_BillingRateAdjustments,
        BILLING.U_ActualDemurrage AS pr_U_BillingActualDemurrage,
        ...
        TP.U_TPStatus,
        DATEADD(day, 15, POD.U_BookingDate) AS U_Aging,
        POD.U_ISLAND,
        POD.U_ISLAND_D,
        POD.U_IFINTERISLAND,
        ...
        '' AS U_APInvLineNum,
        (SELECT value FROM @PODSubmitDeadline WHERE id = POD.U_BookingNumber) AS U_PODSubmitDeadline,
