...
BEGIN TRY
    ...
    DROP TABLE IF EXISTS TMP_ALL_POD_EXTRACT_$serial
    SELECT
        U_BookingNumber
    INTO TMP_ALL_POD_EXTRACT_$serial
    FROM [@PCTP_POD] WITH (NOLOCK)

    DROP TABLE IF EXISTS TMP_INSERT_ALL_POD_EXTRACT_$serial
    SELECT
        --COLUMNS
        CASE
    ...
    INTO TMP_INSERT_ALL_POD_EXTRACT_$serial
    --COLUMNS
    ...
    FROM TMP_ALL_POD_EXTRACT_$serial WITH (NOLOCK))
    ...
    SELECT
        *
    FROM TMP_INSERT_ALL_POD_EXTRACT_$serial WITH (NOLOCK)
