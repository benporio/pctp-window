...
BEGIN TRY
	...
	DROP TABLE IF EXISTS TMP_CON_POD_EXTRACT_$serial
	SELECT
    U_BookingNumber
INTO TMP_CON_POD_EXTRACT_$serial
FROM [@PCTP_POD] WITH (NOLOCK)
WHERE U_PTFNo = 'PTF2023-2208'

	DROP TABLE IF EXISTS TMP_INSERT_CON_POD_EXTRACT_$serial
	SELECT
        --COLUMNS
        CASE
    ...
    INTO TMP_INSERT_CON_POD_EXTRACT_$serial
    --COLUMNS
...
FROM TMP_CON_POD_EXTRACT_$serial WITH (NOLOCK))
...
SELECT
    *
FROM TMP_INSERT_CON_POD_EXTRACT_$serial WITH (NOLOCK)
